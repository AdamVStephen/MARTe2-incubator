/**
 * @file ComtradeReader.cpp
 * @brief Source file for class ComtradeReader
 * @date 11/08/2017
 * @author Andre Neto
 *
 ...
 * the class ComtradeReader (public, protected, and private). Be aware that some 
...
bool ComtradeReader::AllocateMemory() {
    return true;
}

uint32 ComtradeReader::GetNumberOfMemoryBuffers() {
    return 1u;
}

/*lint -e{715}  [MISRA C++ Rule 0-1-11], [MISRA C++ Rule 0-1-12]. Justification: The signalAddress is independent of the bufferIdx.*/
bool ComtradeReader::GetSignalMemoryBuffer(const uint32 signalIdx,
...
const char8* ComtradeReader::GetBrokerName(StructuredDataI &data,
...
bool ComtradeReader::GetInputBrokers(ReferenceContainer &inputBrokers,
...
bool ComtradeReader::GetOutputBrokers(ReferenceContainer &outputBrokers,
                                  const char8 *const functionName,
                                  void *const gamMemPtr) {
    return false;
}
...
void ComtradeReader::ConvertXAxisSignal() {
...
bool ComtradeReader::Synchronise() {
...
bool ComtradeReader::PrepareNextState(const char8 *const currentStateName,
                                  const char8 *const nextStateName) {
    return true;
}

bool ComtradeReader::Initialise(StructuredDataI &data) {
        ...
        if (interpolate) {
            if (!data.Read("XAxisSignal", xAxisSignalName)) {
                REPORT_ERROR(
                        ...
                        "Interpolate=yes and the XAxisSignal was not specified. This will fail if none of the signals interacting with this ComtradeReader has Frequency > 0");
            }
            if (!data.Read("InterpolationPeriod", interpolationPeriod)) {
                REPORT_ERROR(
                        ErrorManagement::Warning,
                        "Interpolate=yes and the InterpolationPeriod was not specified. This will fail if none of the signals interacting with this ComtradeReader has Frequency > 0");
            }
        }
    }
    if (ok) {
...
bool ComtradeReader::SetConfiguredDatabase(StructuredDataI &data) {
...
ErrorManagement::ErrorType ComtradeReader::OpenFile(StructuredDataI &cdb) {
...
bool ComtradeReader::ReadLineCSVFormat() {
    StreamString token;
    char8 saveTerminator;
...
ErrorManagement::ErrorType ComtradeReader::CloseFile() {
...
const StreamString& ComtradeReader::GetFilename() const {
    return filename;
}

const StreamString& ComtradeReader::GetFileFormat() const {
    return fileFormatStr;
}

const StreamString& ComtradeReader::GetCSVSeparator() const {
    return csvSeparator;
}

bool ComtradeReader::IsInterpolate() const {
    return interpolate;
}

const StreamString& ComtradeReader::GetXAxisSignal() const {
    return xAxisSignalName;
}

uint64 ComtradeReader::GetInterpolationPeriod() const {
    return interpolationPeriod;
}

CLASS_REGISTER(ComtradeReader, "1.0")
CLASS_METHOD_REGISTER(ComtradeReader, CloseFile)
